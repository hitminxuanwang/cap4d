...
from gaussianavatars.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from gaussianavatars.utils.sh_utils import RGB2SH
from gaussianavatars.utils.system_utils import mkdir_p
from gaussianavatars.utils.graphics_utils import BasicPointCloud
from gaussianavatars.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def get_xyz(self):
        if self.binding is None:
            ...
            xyz = self.compute_face_xyz_transformed(self.face_center, self.face_orien_mat, self.face_scaling)
            return xyz

    def compute_face_xyz_transformed(self, face_center, face_orien_mat, face_scaling):
        xyz = torch.bmm(face_orien_mat[self.binding], self._xyz[..., None]).squeeze(-1)
        return xyz * face_scaling[self.binding] + face_center[self.binding]
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        scales = torch.ones((self.get_xyz.shape[0], 3), device="cuda")
