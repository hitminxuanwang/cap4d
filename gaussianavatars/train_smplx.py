...
def training_report(
    ...
    if tb_writer and iteration % 10 == 0:
        ...
        if 'lap' in losses:
            lap_value = losses['lap'].detach().item() if isinstance(losses['lap'], torch.Tensor) else losses['lap']
            tb_writer.add_scalar('train_loss_patches/lap', lap_value, iteration)
        if 'deform' in losses:
            deform_value = losses['deform'].detach().item() if isinstance(losses['deform'], torch.Tensor) else losses['deform']
            tb_writer.add_scalar('train_loss_patches/deform', deform_value, iteration)
            #tb_writer.add_scalar('train_loss_patches/deform', losses['deform'].detach().item(), iteration)
        if 'rot' in losses:
            rot_value = losses['rot'].detach().item() if isinstance(losses['rot'], torch.Tensor) else losses['rot']
            tb_writer.add_scalar('train_loss_patches/rot', rot_value, iteration)
