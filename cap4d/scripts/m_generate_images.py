...
REF_DATA_PATH = Path("./data/reference.json")
CONFIG_PATH = Path("./configs/generation/multi_ref.yaml")
OUTPUT_PATH = Path("./outputs/run1")
DEVICE = "cuda"
...
@torch.no_grad()
def extract_conditions(first_rank_model, ref_dataloader, gen_dataloader):
    ...
    print(f"Loading generation dataset from {OmegaConf.load(CONFIG_PATH)['generation_data']['data_path']}")
    ...
    if VISUALIZE_CONDITIONING:
        ...
        save_visualization(ref_data["cond_vis_frames"], load_config_and_paths()[1])
        save_visualization(gen_data["cond_vis_frames"], load_config_and_paths()[2])
    print("Saving flame parameters")
    save_flame_params(ref_data["flame_params"], load_config_and_paths()[1])
    save_flame_params(gen_data["flame_params"], load_config_and_paths()[2])
    gc.collect()
...
def main():
    ...
    ref_data, gen_data = extract_conditions(first_rank_model, ref_dataloader, gen_dataloader)
